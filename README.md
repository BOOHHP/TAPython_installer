...
当前版本已迁移到 **WPF / .NET 8**，采用深色工作台界面：左侧负责项目、引擎和安装配置，右侧展示准备状态、安装进度、操作流程和可滚动安装日志，底部保留主操作按钮。
...
The current version has been migrated to **WPF / .NET 8** with a dark workstation-style interface: project, engine, and setup options are grouped on the left; readiness, progress, workflow, and scrollable install logs are shown on the right; primary actions stay in the bottom action area.
