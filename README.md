...
> **推荐下载 `v1.8.0` 或更新版本。** `v1.5.0` 是首个支持应用内一键自更新的版本；`v1.6.0` 开始，“脚本工具”页支持项目工具导入、导出和删除；`v1.7.0` 新增应用内中文更新日志窗口；`v1.8.0` 新增 `tapython-generator` 与 `ue-api-navigator` Agent Skills 一键部署，并显著压缩单文件 EXE 体积。
...
- **深色安装工作台**
  ...
  - 更新日志按钮会在程序中央打开中文版本说明窗口；通过应用内自更新重启新版客户端后，会自动弹出更新日志窗口提示本次版本变化，并使用本地标记文件兜底避免启动参数丢失
...
> **Recommended download: `v1.8.0` or newer.** `v1.5.0` is the first release with in-app one-click self-update support. Starting from `v1.6.0`, the Script Tools page supports project tool import, export, and deletion. `v1.7.0` adds an in-app Chinese changelog window. `v1.8.0` adds one-click deployment for the `tapython-generator` and `ue-api-navigator` Agent Skills and significantly reduces the single-file EXE size.
...
- **Dark installer workspace**
  ...
  - The changelog button opens a centered Chinese release-notes window; after an in-app self-update relaunches the new client, the changelog opens automatically, with a local marker-file fallback in case startup arguments are lost
