...
> **推荐下载 `v1.9.0` 或更新版本。** `v1.5.0` 是首个支持应用内一键自更新的版本；`v1.6.0` 开始，“脚本工具”页支持项目工具导入、导出和删除；`v1.7.0` 新增应用内中文更新日志窗口；`v1.8.0` 新增 `tapython-generator` 与 `ue-api-navigator` Agent Skills 一键部署，并显著压缩单文件 EXE 体积；`v1.8.1` 开始，客户端会在本机首次打开当前版本时自动展示更新日志；`v1.9.0` 接入 TAPython Tool Hub API 浏览页。
...
- **自动配置**
  ...
  - 配置 Python 附加路径
  - 部署 `tapython-generator` / `ue-api-navigator` 到当前用户 `.copilot/skills`
...
- **TAPython Tool Hub 浏览**
  - “TAPython 工具分享网站”分页直接读取公司内网 Tool Hub API：`http://10.2.13.8:8787/api/tools`
  - 支持按名称/说明/作者/标签搜索，并按分类、风险等级、审核状态和当前 UE 兼容性筛选
  - 展示工具版本、作者团队、兼容 UE、包体积、sha256 摘要、标签和说明
  - 支持读取安装计划模板，预览目标路径、文件数量、MenuConfig 新增项、风险提示和安装后操作；当前阶段仅预览，不写入项目文件
- **Agent Skills 部署**
  - 安装器内置 `tapython-generator` 与 `ue-api-navigator` 两个 Skill，无需目标用户提前安装
...
4. 勾选所需选项，点击 **一键安装 TAPython**；如勾选 Agent Skills，安装器会同步部署 `tapython-generator` 与 `ue-api-navigator` 到当前用户 `.copilot/skills`；如果项目中已有 `TA/TAPython/Python` 脚本，安装器会先识别并在日志中列出当前已有工具，同时避免覆盖用户文件
...
> **Recommended download: `v1.9.0` or newer.** `v1.5.0` is the first release with in-app one-click self-update support. Starting from `v1.6.0`, the Script Tools page supports project tool import, export, and deletion. `v1.7.0` adds an in-app Chinese changelog window. `v1.8.0` adds one-click deployment for the `tapython-generator` and `ue-api-navigator` Agent Skills and significantly reduces the single-file EXE size. Starting from `v1.8.1`, the client opens the changelog automatically the first time the current version is launched on the machine. `v1.9.0` adds the TAPython Tool Hub API browser.
...
- **Auto configuration**
  ...
  - Deploy `tapython-generator` / `ue-api-navigator` into the current user's `.copilot/skills`
...
- **TAPython Tool Hub browsing**
  - The **TAPython Tool Hub** tab reads the internal Tool Hub API directly from `http://10.2.13.8:8787/api/tools`
  ...
  - Reads install-plan templates to preview target path, file count, MenuConfig additions, risk notes, and post-install steps; this stage is preview-only and does not write project files
- **Agent Skills deployment**
  - Bundles `tapython-generator` and `ue-api-navigator` so target users do not need to install them beforehand
...
4. Select desired options and click **Install TAPython**; when Agent Skills are checked, the installer also deploys `tapython-generator` and `ue-api-navigator` to the current user's `.copilot/skills`; if the project already contains `TA/TAPython/Python` scripts, the installer detects them, lists the current tools in the log, and avoids overwriting user files
